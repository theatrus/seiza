...
use crate::{Error, LinearImageMut, LinearImageRef, Result, validate_dimensions};
use seiza_imgproc::{
    BorderMode,
...
impl ResidualFlatPatch {
    ...
    pub fn from_response(
        ...
        validate_dimensions(response.len(), width, height, channels)?;
    ...
    /// The normalized multiplicative response samples.
    pub fn response(&self) -> &[f32] {
        &self.response
    }

    /// Response width in pixels.
    pub fn width(&self) -> usize {
        self.width
    }

    /// Response height in pixels.
    ...
    /// Consume the patch and return its dimensions and response samples.
    pub fn into_parts(self) -> (usize, usize, usize, Vec<f32>) {
        (self.width, self.height, self.channels, self.response)
    }

    /// Divide this response out of a same-sampling image at a detector origin.
    pub fn apply_at(&self, image: LinearImageMut<'_>, x: usize, y: usize) -> Result<()> {
        apply_validated_response_at(
            &self.response,
            self.width,
            self.height,
            self.channels,
...
/// Validate a caller-owned normalized residual response.
pub fn validate_residual_flat_response(response: LinearImageRef<'_>) -> Result<()> {
    validate_response_values(response.data())
}

/// Apply a caller-owned normalized response at a detector origin.
pub fn apply_residual_flat_response_at(
    response: LinearImageRef<'_>,
    image: LinearImageMut<'_>,
    x: usize,
    y: usize,
) -> Result<()> {
    validate_residual_flat_response(response)?;
    apply_validated_response_at(
        response.data(),
        response.width(),
        response.height(),
...
fn apply_validated_response_at(
    response: &[f32],
    response_width: usize,
    response_height: usize,
    response_channels: usize,
    mut image: LinearImageMut<'_>,
    x: usize,
    y: usize,
) -> Result<()> {
    if image.channels() != response_channels {
        return Err(residual_error(format!(
            "light has {} channels but residual-flat patch has {response_channels}",
            image.channels()
        )));
    }
    let end_x = x
        .checked_add(response_width)
        .ok_or_else(|| residual_error("residual-flat horizontal bounds overflow"))?;
    let end_y = y
        .checked_add(response_height)
    ...
    let image_channels = image.channels();
    let image = image.data_mut();
    for patch_y in 0..response_height {
        let light_start = ((y + patch_y) * image_width + x) * image_channels;
        let patch_start = patch_y * response_width * response_channels;
        let sample_count = response_width * response_channels;
        let light_row = &mut image[light_start..light_start + sample_count];
        let response_row = &response[patch_start..patch_start + sample_count];
...
fn validate_response_values(response: &[f32]) -> Result<()> {
    if response
        .iter()
...
pub fn build_residual_flat_patch(
    ...
    let patch = ResidualFlatPatch::from_response(
        response,
        reference.width,
        reference.height,
        reference.channels,
...
fn normalize_sample(image: &LinearImageRef<'_>, options: &ResidualFlatOptions) -> Result<Vec<f32>> {
...
fn residual_error(message: impl Into<String>) -> Error {
    Error::ResidualFlat(message.into())
}
...
mod tests {
    ...
    impl TestImage {
        fn new(width: usize, height: usize, channels: usize, data: Vec<f32>) -> Self {
            LinearImageRef::new(&data, width, height, channels).unwrap();
            Self {
                width,
                height,
                channels,
        ...
        fn as_ref(&self) -> LinearImageRef<'_> {
            LinearImageRef::new(&self.data, self.width, self.height, self.channels).unwrap()
        }
    }

    fn build(samples: &[TestImage], options: &ResidualFlatOptions) -> Result<ResidualFlatBuild> {
        let samples = samples.iter().map(TestImage::as_ref).collect::<Vec<_>>();
        build_residual_flat_patch(&samples, options)
    }

    fn sample_with_shadow(width: usize, height: usize, offset: f32, shadow: bool) -> TestImage {
        let data = (0..height)
            .flat_map(|y| {
                (0..width).map(move |x| {
            ...
            .collect();
        TestImage::new(width, height, 1, data)
    }
    ...
    fn builds_and_applies_a_bounded_repeated_shadow() {
        ...
        let built = build(&samples, &options).unwrap();
        ...
        built
            .patch
            .apply_at(
                LinearImageMut::new(&mut light.data, light.width, light.height, light.channels)
                    .unwrap(),
                4,
                4,
            )
    ...
    fn rejects_a_shadow_without_cross_frame_consensus() {
        let samples = (0..7)
            ...
            .collect::<Vec<_>>();
        let result = build(&samples, &ResidualFlatOptions::default());
    ...
    fn rejects_scattered_repeated_pixels_without_a_coherent_region() {
        ...
        let error = build(&samples, &options).unwrap_err().to_string();
    ...
    fn removes_disconnected_corrections_from_an_accepted_patch() {
        ...
        assert_eq!(built.patch.response()[10 * 64 + 10], 1.0);
        assert!(built.patch.response()[30 * 64 + 30] < 1.0);
    }
        ...
        let samples = (0..7)
            .map(|index| {
                ...
                let data = (0..48)
                    ...
                    .collect();
                TestImage::new(48, 48, 1, data)
            })
            .collect::<Vec<_>>();

        let built = build(&samples, &ResidualFlatOptions::default()).unwrap();
    ...
    fn rejects_too_few_samples_and_invalid_cached_response() {
        ...
        assert!(build(&samples, &ResidualFlatOptions::default()).is_err());
        assert!(ResidualFlatPatch::from_response(vec![1.0, 0.0, 1.1, f32::NAN], 2, 2, 1).is_err());
    }

    #[test]
    fn validates_patch_bounds_and_channels_before_mutating() {
        let patch = ResidualFlatPatch::from_response(vec![0.9; 4], 2, 2, 1).unwrap();
        let mut rgb = TestImage::new(4, 4, 3, vec![1.0; 48]);
        let original = rgb.clone();
        assert!(
            patch
                .apply_at(
                    LinearImageMut::new(&mut rgb.data, rgb.width, rgb.height, rgb.channels)
                        .unwrap(),
                    0,
                    0,
                )
        ...
        let mut mono = TestImage::new(4, 4, 1, vec![1.0; 16]);
        let original = mono.clone();
        assert!(
            patch
                .apply_at(
                    LinearImageMut::new(&mut mono.data, mono.width, mono.height, mono.channels)
                        .unwrap(),
                    3,
                    3,
                )
